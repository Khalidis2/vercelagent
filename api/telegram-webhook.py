...
def append_row(svc, sheet, row):
    try:
        ...
        print(f"[ERROR] append_row({sheet}): {e}")
        return False
...
def update_inventory(svc, item_name, qty_delta, item_type="", notes=""):
    ...
    try:
        ...
        print(f"[ERROR] update_inventory({item_name}): {e}")
        raise RuntimeError("فشل تحديث المخزون في Google Sheets")
...
def log_learning(svc, user_id, user_name, message, intent_detected, was_successful, correction=""):
    try:
        append_row(svc, S_LEARNING, [
            now_str(), str(user_id), user_name, message,
            intent_detected, "success" if was_successful else "failed", correction
        ])
    except Exception:
        pass
...
def log_pattern(svc, user_id, user_name, pattern_type, pattern_data):
    try:
        append_row(svc, S_PATTERNS, [
            now_str(), str(user_id), user_name, pattern_type,
    ...
    try:
        ...
        for r in rows:
            if len(r) >= 6 and r[1] == str(user_id) and r[5] == "success":
                examples.append({"message": r[3], "intent": r[4]})
    ...
    try:
        ...
        if len(similar) >= 3:
            ...
            pattern = {
                ...
                "frequency": len(similar), "avg_amount": round(avg_amount, 2),
                "last_amount": amount
            }
...
def add_transaction(svc, ttype, item, category, amount, user):
    ok = append_row(svc, S_TRANSACTIONS, [now_str(), ttype, item, category, amount, user])
    if not ok:
        raise RuntimeError("فشل حفظ المعاملة في Google Sheets")
...
def get_inventory_qty(inv, item_name):
    name = item_name.strip()
    for x in inv:
        if x["item"].strip() == name:
            return x["qty"]
    for x in inv:
        if name in x["item"]:
            return x["qty"]
    for x in inv:
        if x["item"].strip() and x["item"].strip() in name:
            return x["qty"]
    return 0
...
def get_smart_suggestion(svc, user_id, user_name, item):
    try:
        ...
        for pattern in reversed(recurring):
...
- "كم ربحنا من البيض هالأسبوع؟" → smart_query, query_type=profit_by_item
- "شو أكثر شي بعناه؟" → smart_query, query_type=top_selling
- "كم صرفنا على العلف هالشهر؟" → smart_query, query_type=expense_by_item

أنواع: profit_by_item | expense_by_item | top_selling | top_expense | comparison

════════════════════════════════════════════════════════════════════

🔀 AMBIGUOUS INTENTS:
ambiguous_sale | ambiguous_purchase | ambiguous_expense

🔄 INCOMPLETE: incomplete_purchase | incomplete_sale | incomplete_info

📌 INTENT RULES:
دخل (add_income): بيع منتجات (بيض، لبن، صوف) ليس حيوانات حية
مصروف (add_expense): شراء (أعلاف، دواء، كهرباء، وقود، صيانة)
مواشي-شراء (add_livestock): شراء حيوانات كبيرة
مواشي-بيع (sell_livestock): بيع حيوانات كبيرة
دواجن-شراء (add_poultry): شراء دواجن
...
🔢 مرادفات:
دجاج=فراخ=طيور | غنم=خرفان | بيض=بيظ | لبن=حليب | بعنا=بعت=بيع | جبنا=اشترينا

⚡ CONFIDENCE: أضف confidence 0-1، إذا < 0.6 → intent = "clarify"
        ...
        if ctx.get("waiting_for"):
            ...
            if ctx.get("waiting_for") == "clarification":
                ...
                if is_record and prev_type == "sale":
                    clear_context(user_id); return {"intent": "incomplete_sale", "confidence": 1.0}
                elif is_view and prev_type == "sale":
                    clear_context(user_id); return {"intent": "income_total", "period": "month", "confidence": 1.0}
                elif is_record and prev_type == "purchase":
                    clear_context(user_id); return {"intent": "incomplete_purchase", "confidence": 1.0}
                elif is_view and prev_type == "purchase":
                    clear_context(user_id); return {"intent": "expense_total", "period": "month", "confidence": 1.0}
                elif is_record and prev_type == "expense":
                    clear_context(user_id); return {"intent": "incomplete_info", "needs_clarification": "item", "confidence": 1.0}
...
def h_add_income(svc, d, chat_id, user_name, user_id):
    ...
    try:
        ...
        send(chat_id, "⚠️ ما قدرت أسجل العملية، في مشكلة بـ Google Sheets. جرب مرة ثانية.")
        return
...
def h_sell_livestock(svc, d, chat_id, user_name, user_id):
    ...
    for name, q in splits:
        current = get_inventory_qty(inv, name)
        if current < q:
            send(chat_id,
                f"⚠️ ما في كافي في المخزون\n"
                f"{name}: الرصيد الحالي {current}، تبي تبيع {q}\n"
                f"تأكد من العدد وحاول مرة ثانية"
            )
            return
...
def h_sell_poultry(svc, d, chat_id, user_name, user_id):
    ...
    inv = load_inventory(svc)
    current = get_inventory_qty(inv, bird)
    if current < qty:
        send(chat_id,
            f"⚠️ ما في كافي في المخزون\n"
            f"{bird}: الرصيد الحالي {current}، تبي تبيع {qty}\n"
            f"تأكد من العدد وحاول مرة ثانية"
        )
        return
...
def h_pay_salary(svc, d, chat_id, user_name, user_id):
    ...
    try:
        ...
        send(chat_id, "⚠️ ما قدرت أسجل الراتب، في مشكلة بـ Google Sheets. جرب مرة ثانية.")
        return
...
def h_correction(svc, d, chat_id, user_name, user_id):
    send(chat_id,
        "⚠️ التعديل التلقائي غير متاح حالياً\n"
        "لتصحيح عملية، أضف العملية الصحيحة من جديد\n"
        "وتجاهل السابقة، أو عدّلها مباشرة في جوال Google Sheets"
    )
...
def h_smart_query(svc, d, data, chat_id, user_name, user_id):
    ...
    if query_type == "profit_by_item":
        ...
        send(chat_id,
            ...
            f"الربح: {fmt(income - expense)} درهم\n{D}"
        )
        ...
        for x in income_data:
            items[x["item"]] = items.get(x["item"], 0) + x["amount"]
        if not items:
            send(chat_id, "ما في مبيعات في هالفترة"); return
        ...
        for x in expense_data:
            items[x["item"]] = items.get(x["item"], 0) + x["amount"]
        if not items:
            send(chat_id, "ما في مصاريف في هالفترة"); return
        sorted_items = sorted(items.items(), key=lambda x: -x[1])[:5]
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if confidence < 0.6 and intent not in ["incomplete_purchase", "incomplete_sale", "incomplete_info", "ambiguous_sale", "ambiguous_purchase", "ambiguous_expense"]:
            ...
            send(chat_id, "مافهميت عليك شتقصد 🤔\nممكن تعيد بطريقة أوضح؟\nمثال: بعت بيض بـ 200")
            self._ok(); return
        ...
        if intent in ["incomplete_purchase", "incomplete_sale", "incomplete_info"]:
        ...
        elif intent == "add_income":        h_add_income(svc, d, chat_id, user_name, user_id)
        elif intent == "add_expense":       h_add_expense(svc, d, chat_id, user_name, user_id)
        elif intent == "add_livestock":     h_add_livestock(svc, d, chat_id, user_name, user_id)
        elif intent == "sell_livestock":    h_sell_livestock(svc, d, chat_id, user_name, user_id)
        elif intent == "add_poultry":       h_add_poultry(svc, d, chat_id, user_name, user_id)
        elif intent == "sell_poultry":      h_sell_poultry(svc, d, chat_id, user_name, user_id)
        elif intent == "pay_salary":        h_pay_salary(svc, d, chat_id, user_name, user_id)
            ...
            h_profit(data, period, chat_id); clear_context(user_id)
        elif intent == "inventory":
            h_inventory(svc, chat_id, d.get("inventory_item") or d.get("item")); clear_context(user_id)
        elif intent == "last_transactions":
            h_last(data, chat_id); clear_context(user_id)
        elif intent == "income_by_item":
            h_income_by_item(data, d, chat_id); clear_context(user_id)
        elif intent == "income_breakdown":
            h_income_breakdown(data, d, chat_id); clear_context(user_id)
        elif intent == "smalltalk":
            h_smalltalk(chat_id); clear_context(user_id)
        else:
            log_learning(svc, user_id, user_name, text, intent, False, "Unhandled intent")
            send(chat_id, "مافهميت عليك 🤔\nجرب:\n• بعت بيض بـ 200\n• كم الربح؟\nاو /help")
            clear_context(user_id)
